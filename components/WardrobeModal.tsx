...
const WardrobePanel: React.FC<WardrobePanelProps> = ({ onGarmentSelect, activeGarmentIds, isLoading, wardrobe, isOnline }) => {
    ...
    <div className="pt-6 border-t border-gray-400/50">
        ...
        <div className="grid grid-cols-3 gap-3">
            ...
            <label 
              ...
              className={cn(
                'relative aspect-square border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-gray-500 transition-colors', 
                (isLoading || !isOnline) 
                  ? 'cursor-not-allowed bg-gray-100 opacity-60' 
                  : 'hover:border-gray-400 hover:text-gray-600 cursor-pointer'
              )}
              title={!isOnline ? "Uploading is disabled while offline" : "Upload a custom garment"}
            >
                <UploadCloudIcon className="w-6 h-6 mb-1"/>
                <span className="text-xs text-center">Upload</span>
                <input id="custom-garment-upload" type="file" className="hidden" accept="image/png, image/jpeg, image/webp, image/avif, image/heic, image/heif" onChange={handleFileChange} disabled={isLoading || !isOnline}/>
            </label>
        </div>
        {wardrobe.length === 0 && (
