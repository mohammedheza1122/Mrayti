...
import { RotateCcwIcon, ChevronLeftIcon, ChevronRightIcon, MaximizeIcon, DownloadIcon } from './icons';
...
interface CanvasProps {
  ...
  onEnhanceAndSave: () => void;
  isOnline: boolean;
}

const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys, onEnhanceAndSave, isOnline }) => {
  ...
  const handleResetZoom = () => {
    if (transformComponentRef.current) {
        transformComponentRef.current.resetTransform(200); // 200ms animation
    }
  };
        ...
        {displayImageUrl && !isLoading && (
            <>
                <button
                    ...
                    className="flex items-center justify-center text-center bg-white/60 border border-gray-300/80 text-gray-700 p-2.5 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-gray-400 active:scale-95 backdrop-blur-sm"
                >
                    <MaximizeIcon className="w-4 h-4" />
                </button>
                <button
                    onClick={onEnhanceAndSave}
                    aria-label="Enhance and save image"
                    disabled={!isOnline}
                    title={!isOnline ? "Unavailable offline" : "Enhance and save image"}
                    className="flex items-center justify-center text-center bg-white/60 border border-gray-300/80 text-gray-700 p-2.5 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-gray-400 active:scale-95 backdrop-blur-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {displayImageUrl ? (
          <TransformWrapper
            ...
            wheel={{ step: 0.2 }}
            panning={{ disabled: isLoading, velocityDisabled: true }}
            zoomAnimation={{ duration: 250, easing: "easeOutQuad" }}
          >
            <TransformComponent
            ...
            >
                <img
                    ...
                    className="max-w-full max-h-full object-contain transition-opacity duration-500 animate-fade-in rounded-lg cursor-grab active:cursor-grabbing"
                />
            </TransformComponent>
          </TransformWrapper>
        ) : (
