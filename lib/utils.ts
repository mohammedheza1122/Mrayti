...
export function getFriendlyErrorMessage(error: unknown, context: string): string {
    ...
    if (rawMessage.includes("Unsupported MIME type")) {
        try {
            ...
            // FIX: Add type check for nestedMessage to ensure it's a string before calling string methods.
...
export async function dbSet(key: IDBValidKey, value: any): Promise<void> {
...
export async function dbDel(key: IDBValidKey): Promise<void> {
  const db = await getDB();
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(STORE_NAME, 'readwrite');
    const store = transaction.objectStore(STORE_NAME);
